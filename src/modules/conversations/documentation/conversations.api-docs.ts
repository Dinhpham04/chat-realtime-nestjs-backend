...
/**
 * Activate Conversation API Documentation
 * 
 ...
 */
export function ActivateConversationApiDocs() {
  return applyDecorators(
    ApiOperation({
      summary: 'Activate conversation with first message',
      description: `
            Activate a dormant conversation by sending the first message.
            This transforms the conversation from dormant to active state.
            
            **Use Cases:**
            - Send first message in new conversation
            - Activate dormant conversation
            - Send message with attachments as first message
            
            **Features:**
            - Supports text messages with mentions
            - Supports file attachments (images, documents, etc.)
            - Atomically activates conversation and sends message
            - Returns full conversation context
            
            **Mobile UX Pattern:**
            - Single API call for activate + send message
            - Atomic operation - no partial state
            - Real-time notification to other participant
            `
    }),
    ApiBody({
      type: ActivateConversationDto,
      description: 'Conversation ID and initial message data',
      examples: {
        text_message: {
          summary: 'Text message activation',
          description: 'Activate with simple text message',
          value: {
            conversationId: '64f2b3c4d5e6f7a8b9c0d2e3',
            initialMessage: {
              content: {
        ...
        image_message: {
          summary: 'Image message activation',
          description: 'Activate with image attachment',
          value: {
            conversationId: '64f2b3c4d5e6f7a8b9c0d2e3',
            initialMessage: {
              content: {
    ...
    ApiResponse({
      status: HttpStatus.OK,
      description: 'Conversation activated successfully with first message',
      type: ActivateConversationResponseDto,
      examples: {
        successful_activation: {
          summary: 'Successful conversation activation',
          value: {
            conversation: {
              id: '64f2b3c4d5e6f7a8b9c0d2e3',
              type: 'direct',
